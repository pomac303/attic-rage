...
static int
gtk_xtext_text_width_8bit (GtkXText *xtext, char *str, int len)
{
	int width = 0;
...
backend_font_open (GtkXText *xtext, char *name)
{
	...
	if (pango_font_description_get_size (fontd) != 0)
	{
		...
		font = XftFontOpen (xdisplay, screen,
				...
				XFT_CORE, XftTypeBool, False, XFT_SIZE, XftTypeDouble, 
				(double)pango_font_description_get_size (fontd)/PANGO_SCALE,
				XFT_WEIGHT, XftTypeInteger, weight,
				XFT_SLANT, XftTypeInteger, slant, NULL);
	}
...
inline static int
backend_get_char_width (GtkXText *xtext, char *str, int *mbl_ret)
{
	XGlyphInfo ext;
		...
		char *str, int len, int str_width, int is_mb)
{
	...
	if (xtext->bold)
		gdk_draw_layout_line_with_colors (xtext->draw_buf, gc, x + 1, y, line, 0, 0);
#endif
}
...
gtk_xtext_realize (GtkWidget * widget)
{
	...
	xtext->hand_cursor = gdk_cursor_new_for_display (
			gdk_drawable_get_display (widget->window), GDK_HAND1);
	xtext->resize_cursor = gdk_cursor_new_for_display (
			gdk_drawable_get_display (widget->window), GDK_LEFT_SIDE);
#endif
...
lamejump:
		...
		while (ent)
		{
			...
			if (ent == end_ent)
			{
				gtk_xtext_selection_down (xtext, ent, 
						xtext->buffer->last_ent_end, end_offset);
			...
			xtext->buffer->last_offset_end == end_offset)
	{
		...
		while (ent)
		{
			if (ent == start_ent)
			{
				gtk_xtext_selection_up (xtext, xtext->buffer->last_ent_start, 
						ent, start_offset);
...
gtk_xtext_class_init (GtkXTextClass * class)
{
	...
	xtext_signals[WORD_CLICK] =
		g_signal_new ("word_click",
				...
				NULL, NULL, gtk_marshal_VOID__POINTER_POINTER,
...
static int
gtk_xtext_text_width (GtkXText *xtext, char *text, int len, int *mb_ret)
{
	char *new_buf;
	int new_len, mb;
...
static int
gtk_xtext_render_flush (GtkXText * xtext, int x, int y, char *str,
		int len, GdkGC *gc, int is_mb)
{
	...
	if (x > xtext->clip_x2 || x + str_width < xtext->clip_x)
		return str_width;
	if (y - xtext->font->ascent > xtext->clip_y2 || (y - xtext->font->ascent) + 
			xtext->fontsize < xtext->clip_y)
		return str_width;
...
gtk_xtext_render_str (GtkXText * xtext, int y, textentry * ent,
		char *str, int len, int win_width, int indent,
	...
	if (img->depth <= 14)
	{
		...
		for (y = 0; y < height; y++)
		{
			for (x = 0; x < width; x++)
			{
				if (img->depth == 1)
				{
					pixel = (((guchar *) img->mem)[y * img->bpl + 
							(x >> 3)] & (1 << (7 - (x & 0x7)))) != 0;
					goto here;
				}
...
gtk_xtext_load_trans (GtkXText * xtext)
{
	...
	rootpix = get_pixmap_prop (GDK_WINDOW_XDISPLAY (widget->window), 
...
#else
		xtext->pixmap = gdk_pixmap_foreign_new_for_display (
				gdk_drawable_get_display (GTK_WIDGET (xtext)->window), rootpix);
#endif
		gdk_gc_set_tile (xtext->bgc, xtext->pixmap);
...
static int
find_next_wrap (GtkXText * xtext, textentry * ent, char *str,
		int win_width, int indent)
{
	char *last_space = str, *orig_str = str;
		...
		if (!subline)
		{
						...
						indent, line, FALSE))
			{
				...
				gtk_xtext_draw_marker (xtext, ent, 
						y - xtext->fontsize * (taken + start_subline + 1));
...
void *
gtk_xtext_search (GtkXText * xtext, const char *text, void *start)
{
	textentry *ent, *fent;
	char *str;
	int line;
...
void
gtk_xtext_append (xtext_buffer *buf, char *text, int len)
{
	textentry *ent;
